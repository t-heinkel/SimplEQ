...
void RotarySliderWithLabels::paint(juce::Graphics &g)
{
    ...
    // slider values have to be in radiant
    ...
    //Adds the rectangles behind the circles 
    ...
    getLookAndFeel().drawRotarySlider(g, 
                                    ...
                                    jmap(getValue(), range.getStart(), range.getEnd(), 0.0, 1.0), // turns slider-value into normalized value
                                    startAng, 
                                    endAng, 
                                    *this);
    ...
    for (int i = 0; i < numChoices; ++i)
    {
        ...
        // gets Point on specific length, at specific angle away from centre 
...
juce::String RotarySliderWithLabels::getDisplayString() const
{
    ...
    if (auto* floarParam = dynamic_cast<juce::AudioParameterFloat*>(param))
    {
        float val = getValue();
        ...
        {
            val /= 1000.f;
            addK = true; // display kHz
        }

        str = juce::String(val, (addK ? 2 : 0)); // formatting String, if addK is true set 2 decimal places, otherwise 0
    }
    else
    {
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
   ...
   */
    ...
    /*
    * while there are paths that can pull
...
void ResponseCurveComponent::paint(juce::Graphics& g)
{
    ...
    std::vector<double> mags; //Stores the magnitudes per pixel
...
juce::Rectangle<int> ResponseCurveComponent::getRenderArea()
{
    ...
    //bounds.reduce(10, //JUCE_LIVE_CONSTANT(5),        // just one live constant per line!
...
void SimplEQAudioProcessorEditor::resized()
{
    ...
    float hRatio = 25.f / 100.f; //JUCE_LIVE_CONSTANT(33) / 100.f; is able to live show the effect when changing the value in parantheses
